...
def openai_extract_title_author(image_bytes):
    ...
    system_prompt = (
        ...
        "Si no hay autor, devuelve cadena vacía. Ejemplo: {\"titulo\":\"Cien años de soledad\",\"autor\":\"Gabriel García Márquez\"}"
    )
    try:
...
with col_ctrl:
    ...
    st.markdown("**1) Capturar ubicación inicio**")
    if st.button("📍 Capturar ubicación inicio"):
        ...
        if coords:
            ...
            st.error("No se pudo obtener la ubicación. Da permiso al navegador.")

    st.markdown("---")
    st.markdown("**2) Subir foto de portada y detectar título/autor (GPT-4o)**")
    uploaded = st.file_uploader("Foto (jpg/jpeg/png)", type=["jpg","jpeg","png"])
    titulo_sugerido = ""
    autor_sugerido = ""
    ...
    if uploaded:
        ...
        if st.button("🔎 Detectar título y autor"):
            ...
            with st.spinner("Analizando imagen con GPT-4o..."):
                titulo_sugerido, autor_sugerido = openai_extract_title_author(image_bytes)
            if titulo_sugerido or autor_sugerido:
                st.success("Detección completada.")
            else:
                st.warning("No se detectó título/autor confiablemente.")

    titulo = st.text_input("Título (editar o confirmar)", value=titulo_sugerido)
    autor = st.text_input("Autor (editar o confirmar)", value=autor_sugerido)

    st.markdown("---")
    st.markdown("**3) Cronómetro de lectura manual**")
    ...
    if not st.session_state["reading_started"]:
        if st.button("▶️ Iniciar lectura"):
            if "start_coords" not in st.session_state:
                st.error("Primero capturá la ubicación de inicio.")
            else:
        ...
        if st.button("⏹ Detener lectura"):
            ...
            if not coords_end:
                st.error("No se pudo obtener ubicación final.")
            else:
                ...
                registro = {
                    ...
                    "inicio_ts_utc": datetime.utcnow(),
                    "inicio_ts_local": now_local.isoformat(),
                    "inicio_coords": {"lat": start_coords[0], "lon": start_coords[1]},
                    "fin_ts_utc": datetime.utcnow(),
                    "fin_ts_local": datetime.now(bogota).isoformat(),
    ...
    if mongo_collection:
        try:
            docs = list(mongo_collection.find().sort("inicio_ts_utc", -1).limit(30))
            for d in docs:
                historia.append({
                    "titulo": d.get("titulo",""),
                    "autor": d.get("autor",""),
